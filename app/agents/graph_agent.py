...
- **局部更新**：如果已有计划，在已有基础上调整，不要全部重写
- **工具调用参数规范**：在调用 `tool_generate_plan` 时，传入 `user_input` 的参数必须是干净的**自然语言学习目标或目标主题**（如 "考研冲刺" 或 "高考数学"），**绝对不要**传入任何 JSON 格式的字符串、工具调用原始输出或上下文数据结构。
